...
export async function updateSurveyAction(
    ...
    try {
        ...
        if (existingSurvey.creatorId !== session.user.id) {
            return { success: false, message: "У вас нет прав на редактирование этого опроса." };
        }
        ...
        return { success: false, message: "Внутренняя ошибка сервера. Не удалось сохранить опрос." };
    }
}
