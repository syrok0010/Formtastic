...
export async function submitSurvey(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const answerCreationPromises = Object.entries(answers).map(
        async ([questionIdStr, value]) => {
          const questionId = parseInt(questionIdStr, 10);
          const questionType = questionTypeMap.get(questionId);

          if (!questionType) {
            console.warn(
              `Вопрос с ID ${questionId} не найден в опросе. Пропускаем.`,
            );
            return;
          }
    ...
    return {
      success: false,
      error: "Внутренняя ошибка сервера. Пожалуйста, попробуйте еще раз.",
    };
  }
}
